...
// Keep the real shell, navigation, project filters and contact section. Stub
// only unrelated sections and artwork.
...
describe("navigation and persistent UI state", () => {
   it("keeps the contact section mounted and focus intact through both motion modes", async () => {
      render(<App />);
      fireEvent.click(
         screen.getByRole("button", { name: "Navigate to Contact" }),
      );
      const emailLink = await screen.findByRole(
         "link",
   ...
   it("loads and focuses a direct contact link after preceding sections mount", async () => {
      ...
      expect(document.querySelectorAll("[data-section-ready]")).toHaveLength(8);
      expect(
   ...
   it("handles browser history navigation without reloading the application", async () => {
      ...
      // The contact section stays mounted (not torn down) once it has rendered.
      expect(emailLink.isConnected).toBe(true);
   });
});
