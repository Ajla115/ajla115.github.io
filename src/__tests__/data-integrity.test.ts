...
describe("portfolio data invariants", () => {
   ...
   it("keeps discussion identifiers unique", () => {
   ...
   it("keeps Credly-verified badge identifiers unique and hosted on Credly", () => {
      // Only entries with a badgeId are Credly-verified; a plain-text
      // certification (no badgeId) is exempt from the host checks below.
      const badges = [...getCertifications(), ...getLearningBadges()].filter(
         (badge) => badge.badgeId,
      );
