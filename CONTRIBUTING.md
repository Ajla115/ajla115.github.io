...
This is a React and TypeScript portfolio with JSON content, reusable UI components, and eight page sections. Start with the file map below, then follow the recipe for the change you want to make.
...
Open [localhost:3000](http://localhost:3000/). The root `/` base path is intentional: this repo deploys to a `<username>.github.io` user page, which GitHub Pages serves at the domain root.
...
| Change                                                  | Start here                                                                                                                       |
| ------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| Name, introduction, bio, or social links                | [data/personal.json](data/personal.json)                                                                                         |
...
| Contact links                                           | [data/contact.json](data/contact.json)                                                                                           |
...
The hero's latest-work line derives its value from the content. Update the source records instead of copying those values into components.
...
Experience and education cards fall back to a generic building icon unless you add a real logo. [orgLogos.tsx](src/utils/orgLogos.tsx) matches a company/institution name to a mark; add a WebP image to `src/assets/logos/`, import it there, and register the name (see [src/assets/logos/README.md](src/assets/logos/README.md) for sourcing notes). Updating this mapping updates both the cards and experience detail headers.
...
2. Add the same ID to `COVER_BY_ID` in [coverRegistry.ts](src/pages/projects/covers/coverRegistry.ts). Every project requires exactly one registered cover.
3. Place a 960x600 WebP screenshot in [src/assets/projects/](src/assets/projects), import it in the registry, and use `{ kind: "image", src: importedImage }`. A GitHub repo's own OpenGraph card image (`https://opengraph.githubassets.com/1/<owner>/<repo>`) is a quick, accurate source when you don't have a hand-captured screenshot.
...
Set the top-level `spotlight_project_id` in [data/projects.json](data/projects.json) to the numeric ID of a record in `featured_projects`. For example, `"spotlight_project_id": 1` selects the Agentic AI Automation Systems project.
...
`node_modules/` and `build/` are generated or local files and are ignored by Git. The CV PDF and its page images under `public/` are rendered at build time from the source `AjlaKormanCV.pdf` at the repo root.

Use `pnpm fetch:resume` when working on the CV viewer locally; it renders the pages from the local PDF, so it needs no network access. Normal application development and pull request builds do not require running it. See [scripts/prepare-resume.js](scripts/prepare-resume.js) for that pipeline.

[CI](.github/workflows/ci.yml) runs the code and data checks (format, lint, types, data validation, tests, build) on every push to `main` and every pull request. Deployment is manual -- `pnpm run deploy` builds and pushes `build/` to the `gh-pages` branch. Keep Vite's root `/` base path and `build/` output aligned with that workflow.
