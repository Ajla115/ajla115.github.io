#!/usr/bin/env node
// Render the source CV PDF to high-resolution WebP pages so the in-site CV
// viewer shows crisp, zoomable images with zero client-side PDF machinery.
// Source: AjlaKormanCV.pdf at the repo root. Outputs (gitignored):
//   public/resume.pdf              -- for the open-in-new-tab / download action
...
const SOURCE_PDF = path.join(ROOT, "AjlaKormanCV.pdf");
...
console.log("Reading source CV...");
const pdfBuffer = await readFile(SOURCE_PDF);
if (pdfBuffer.subarray(0, 5).toString("ascii") !== "%PDF-") {
   throw new Error("AjlaKormanCV.pdf is not a valid PDF");
}
await copyFile(SOURCE_PDF, PDF_OUT);
...
for await (const pagePng of document) {
   ...
   // A mostly-white page compresses tightly lossless anyway.
