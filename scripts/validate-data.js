...
if (requireRecord(achievements, "achievements")) {
   ...
   // Certifications are plain records (name/type/issuer) unless the issuer
   // also verifies them on Credly, in which case the badge fields below apply.
   ...
   // Only entries that carry a badgeId are Credly-verified; a plain-text
   // certification with no badgeId skips the uniqueness and host checks below.
   const credlyBadges = allBadges.filter((badge) => badge?.badgeId);
   requireUnique(credlyBadges, "badgeId", "achievements.all_badges");
   for (const [index, badge] of credlyBadges.entries()) {
